...
| `get_blended_portfolio_returns(client_id, risk_profile)` | 1hr | Downloads 5yr monthly ETF prices via yfinance. Returns `(port_returns, bench_returns)` as monthly pd.Series. Portfolio weighted by actual lot market values; benchmark = policy weights for the risk profile. ASSET_ETF map: Equity/Concentrated Stock→SPY, Intl Equity→EFA, Fixed Income→AGG, High Yield→HYG, Real Estate→VNQ, Commodities→PDBC, PE→PSP, Cash→SHV. |
| `get_sleeve_returns(client_id)` | 1hr | Downloads 3yr monthly ETF prices per asset-class sleeve. Returns `{asset_class: {"returns": pd.Series, "etf": str, "value": float}}`. Used by Performance & Holdings sleeve cards. |
...
| `data/client_mapping.py` | Hardcoded client→strategy dict. 6 active clients: Patricia Huang (C002, Equity), Elena Rodriguez (C004, HYB), James Whitfield (C005, Equity), Robert Chen (C007, Equity), Sarah Williams (C008, Equity), Michael Park (C009, Equity). Update this when onboarding new clients. |
| `data/client_transactions.csv` | Lot-level tax lot schema. Columns: lot_id, account_id, client_id, ticker, shares, cost_basis_per_share, current_price, cost_basis_total, current_value, unrealized_gl_dollars, unrealized_gl_pct, purchase_date, term, wash_sale_flag, replacement_ticker, asset_class, notes. Lots for C002/C004/C005/C007/C008/C009. Patricia (C002): NVDA L007/L008/L009 are underwater TLH candidates; L001–L006 have 873–1953% gains (do not harvest). Whitfield (C005): VBTLX L034 and TPINX L035 are largest TLH opps. Park (C009): TLT L048 is a large TLH opp (-38%). |
